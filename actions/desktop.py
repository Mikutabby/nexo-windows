...
def _execute_generated_code(code: str, player=None) -> str:
    ...
    try:
        exec(compile(code, "<nexo_desktop>", "exec"), sandbox)
...
def _find_file_in_dir(name: str, search_path: str = "desktop") -> str | None:
    """Search for a file by name in a directory shortcut, return full path or None."""
    try:
        from actions.file_controller import find_files
        result = find_files(name=name, path=search_path, max_results=5)
        if result and "📄" in result:
            # Parse first result — format: "📄 filename.ext  (1.2 MB)  →  C:\path\to\file"
        ...
        print(f"[Desktop] find_file error: {e}")
        return None
...
def desktop_control(
    ...
    parameters:
        ...
        search_name  : filename to search for in a directory (auto-finds full path)
        search_path  : directory to search in: desktop, downloads, documents, home (default: desktop)
    """
    params = parameters or {}
    action = params.get("action", "").lower().strip()
    task   = params.get("task", "").strip()
    search_name = params.get("search_name", "").strip()
    search_path = params.get("search_path", "desktop").strip()
    ...
    try:
        if action == "wallpaper":
            ...
            # If no direct path but search_name is given, find the file first
            if not path and search_name:
                found = _find_file_in_dir(search_name, search_path)
                if found:
                    path = found
                    if player:
                        player.write_log(f"[desktop] Encontrado: {found}")
                else:
                    return (f"No encontré '{search_name}' en {search_path}. "
                            f"Revisá el nombre o probá con otra ubicación.")
